...
import com.groupcontroldroid.server.bean.CodeBean;
...
public class CodeListener implements DataListener<String>{
	final static Logger logger = LoggerFactory.getLogger(CodeListener.class);
	...
	public void onData(SocketIOClient client, String data, AckRequest ackSender) throws Exception {
		if (data!=null) {
			logger.info("codeListener:" + data);
			CodeBean codeBean = JsonUtil.jsonTobean(data.trim(),
					CodeBean.class);
			if (codeBean.getSerList() != null) {
				 List<String> lists= codeBean.getSerList();
				 String answer =codeBean.getText().trim();
				if (answer != null) {
									...
									if (idevice != null) {
										try {
											...
											outputStream = socket
												...
												infoToAPPBean.setStyle(ApkConfig.INTPUT_CODE);
